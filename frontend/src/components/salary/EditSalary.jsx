...
const EditSalary = ({ isOpen, onClose, onSuccess, salary }) => {
  ...
  // Fetch departments and employees, and populate form
  useEffect(() => {
    if (isOpen && salary) {
      fetchDepartments();
      fetchEmployees();
  ...
  const populateForm = () => {
    if (salary) {
      ...
      const payDate = salary.payDate 
        ? new Date(salary.payDate).toISOString().split('T')[0]
        : '';

      setValue("department", salary.department?._id || salary.department || "");
      setValue("employee", salary.employee?._id || salary.employee || "");
      setValue("basicSalary", salary.basicSalary || 0);
      setValue("allowance", salary.allowance || 0);
      setValue("deduction", salary.deduction || 0);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await axiosInstance.put(`/salary/${salary._id}`, salaryData);
